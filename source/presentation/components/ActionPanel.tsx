...
export const ActionPanel: FC<IActionPanelProps> = ({
  ...
  useEffect(() => {
    ...
    const updatedAction = actions.find(action => action.label === currentAction.label);
      ...
      <Listbox value={currentAction} onChange={setCurrentAction}>
        <div className="col-span-2 relative">
          ...
          <Transition
            ...
            leaveTo="opacity-0"
          >
            <Listbox.Options className="absolute bottom-[calc(100%_+_0.5rem)] mt-1 max-h-60 w-full overflow-auto rounded-md bg-gradient-to-br from-slate-400 to-slate-500 py-1 text-lg shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none sm:text-sm">
              <Listbox.Option key="default-action" value="Select" className="sr-only" disabled>
                Select
              </Listbox.Option>
              {actions.map((action, index) => (
                <Listbox.Option
                  ...
                  className={({ active }) =>
                    ...
                    last:after:content-none after:content-[''] after:absolute after:block after:bottom-0 after:bg-slate-800/25 after:mx-auto after:w-4/5 after:h-px`
                  }
                  value={action}
                >
                  {action.label}
        ...
        <button
          className="btn-full bg-dracula-green hover:bg-transparent from-dracula-green/90 to-dracula-green/50 ring-dracula-green/30"
